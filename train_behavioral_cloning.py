...
# =============================================================================
# 路径 2: 行为克隆 (Behavioral Cloning, BC)
CONFIG = {
    # --- 实验选择 ---
    'EXPERIMENT_NAME': '2_BC_MLP', # 实验名称 (BC 会触发行为克隆)

    # --- 模型架构 ---
    'MODEL_TYPE': 'MLP',      # 'MLP' (4帧堆叠) 或 'RNN' (LSTM)
...
def load_and_preprocess_data(csv_path, train_ids, test_ids):
    ...
    data_info = {}
    data['jerk1'] = 0.0; data['jerk2'] = 0.0; data['jerk3'] = 0.0
...
def calculate_reward(predicted, actual, pred_jerk, actual_jerk, 
    ...
    mse = np.mean((predicted - actual)**2); reward = -mse
    if prev_predicted is not None and prev_actual is not None:
        pred_velocity = predicted - prev_predicted; actual_velocity = actual - prev_actual
        velocity_mse = np.mean((pred_velocity - actual_velocity)**2); reward -= 0.2 * velocity_mse
        actual_direction = np.sign(actual - prev_actual); pred_direction = np.sign(predicted - prev_predicted)
        direction_reward = np.sum(actual_direction == pred_direction) / len(actual_direction); reward += 0.1 * direction_reward
    if use_jerk:
        jerk_mse = np.mean((pred_jerk - actual_jerk)**2); reward -= jerk_weight * jerk_mse
    if is_terminal: reward = reward * 2.0
    reward = reward / (1 + abs(reward)); return reward
...
def get_state_MLP(data, t):
    ...
    for i in range(4):
        idx = max(0, t - i); row = data.iloc[idx]; offset = (3 - i) * 9
...
def get_state_RNN(data, t):
    row = data.iloc[t]
    return row[['joint1', 'joint2', 'joint3','velocity1', 'velocity2', 'velocity3','acceleration1', 'acceleration2', 'acceleration3']].values
def get_jerk_RNN(data, t):
    row = data.iloc[t]; return row[['jerk1', 'jerk2', 'jerk3']].values
def get_action_from_row(row):
    return row[['joint1', 'joint2', 'joint3']].values
def get_current_features(state_vec_9d):
    pos = state_vec_9d[0:3]; vel = state_vec_9d[3:6]; acc = state_vec_9d[6:9]
    return pos, vel, acc
#################################################################
# 缓冲区 (Buffers)
#################################################################
...
class ReplayBuffer:
    def __init__(self, buffer_size, batch_size):
        self.buffer = deque(maxlen=buffer_size); self.batch_size = batch_size
    def add(self, state, action, reward, next_state, done):
        self.buffer.append((state, action, reward, next_state, done))
    def sample(self):
        if len(self.buffer) < self.batch_size: return None
        batch = random.sample(self.buffer, self.batch_size); states, actions, rewards, next_states, dones = zip(*batch)
    ...
    def prefill(self, data):
        ...
        for t in tqdm(range(3, len(data) - 1)):
            ...
            row = data.iloc[t+1]; action = get_action_from_row(row)
            reward = 1.0; done = False
            self.add(state, action, reward, next_state, done)
        print("Buffer prefill complete.")
class ReplayBuffer_RNN:
    def __init__(self, buffer_size, batch_size, seq_len):
        self.buffer = deque(maxlen=buffer_size); self.batch_size = batch_size; self.seq_len = seq_len
        print(f"RNN Replay Buffer initialized with SeqLen: {seq_len}")
    def add(self, state, action, reward, next_state, done):
        self.buffer.append((state, action, reward, next_state, done))
    def sample(self):
        if len(self.buffer) < self.batch_size * self.seq_len: return None
        ...
        for _ in range(self.batch_size):
            ...
            while has_done:
                idx = random.randint(self.seq_len - 1, len(self.buffer) - 1)
                has_done = any(self.buffer[i][4] for i in range(idx - self.seq_len + 1, idx))
            sequence = [self.buffer[i] for i in range(idx - self.seq_len + 1, idx + 1)]; s, a, r, s_next, d = zip(*sequence)
            states_seq.append(s); actions_seq.append(a); rewards_seq.append(r); next_states_seq.append(s_next); dones_seq.append(d)
        return (torch.tensor(np.array(states_seq), dtype=torch.float32).to(device),
...
class StochasticActor_MLP(nn.Module):
    def __init__(self, input_dim, hidden_dim1, hidden_dim2, output_dim, max_action=1.0):
        super(StochasticActor_MLP, self).__init__(); self.bn_eps = 1e-6; self.bn_momentum = 0.1
        self.layer1 = nn.Linear(input_dim, hidden_dim1); self.bn1 = nn.BatchNorm1d(hidden_dim1, eps=self.bn_eps, momentum=self.bn_momentum)
        self.layer2 = nn.Linear(hidden_dim1, hidden_dim2); self.bn2 = nn.BatchNorm1d(hidden_dim2, eps=self.bn_eps, momentum=self.bn_momentum)
        self.mean_layer = nn.Linear(hidden_dim2, output_dim); self.log_std_layer = nn.Linear(hidden_dim2, output_dim)
        self.max_action = max_action; self.init_weights()
    def init_weights(self):
        nn.init.kaiming_normal_(self.layer1.weight, mode='fan_in', nonlinearity='relu'); nn.init.kaiming_normal_(self.layer2.weight, mode='fan_in', nonlinearity='relu')
        nn.init.kaiming_normal_(self.mean_layer.weight, mode='fan_in', nonlinearity='relu'); nn.init.kaiming_normal_(self.log_std_layer.weight, mode='fan_in', nonlinearity='relu')
    def forward(self, x):
        h1 = torch.tanh(self.bn1(self.layer1(x))); h2 = torch.tanh(self.bn2(self.layer2(h1)))
        mean = self.mean_layer(h2); log_std = self.log_std_layer(h2)
        log_std = torch.clamp(log_std, min=-20, max=2); std = torch.exp(log_std); return mean, std
    def sample(self, state):
        mean, std = self.forward(state); dist = Normal(mean, std); z = dist.rsample()
        action = self.max_action * torch.tanh(z)
        log_prob = dist.log_prob(z) - torch.log(self.max_action * (1 - torch.tanh(z).pow(2)) + 1e-6)
        log_prob = log_prob.sum(axis=-1, keepdim=True); return action, log_prob
class Critic_MLP(nn.Module):
    def __init__(self, input_dim, hidden_dim1, hidden_dim2, output_dim=1):
        super(Critic_MLP, self).__init__(); self.bn_eps = 1e-6; self.bn_momentum = 0.1
        self.layer1 = nn.Linear(input_dim, hidden_dim1); self.bn1 = nn.BatchNorm1d(hidden_dim1, eps=self.bn_eps, momentum=self.bn_momentum)
        self.layer2 = nn.Linear(hidden_dim1, hidden_dim2); self.bn2 = nn.BatchNorm1d(hidden_dim2, eps=self.bn_eps, momentum=self.bn_momentum)
        self.layer3 = nn.Linear(hidden_dim2, output_dim); self.init_weights()
    def init_weights(self):
        nn.init.kaiming_normal_(self.layer1.weight, mode='fan_in', nonlinearity='relu'); nn.init.kaiming_normal_(self.layer2.weight, mode='fan_in', nonlinearity='relu')
        nn.init.kaiming_normal_(self.layer3.weight, mode='fan_in', nonlinearity='relu')
    def forward(self, state, action):
        x = torch.cat([state, action], 1); h1 = torch.tanh(self.bn1(self.layer1(x))); h2 = torch.tanh(self.bn2(self.layer2(h1)))
        q_value = self.layer3(h2); return q_value
class StochasticActor_RNN(nn.Module):
    def __init__(self, input_dim, hidden_dim, rnn_hidden_dim, output_dim, max_action=1.0):
        super(StochasticActor_RNN, self).__init__(); self.rnn_hidden_dim = rnn_hidden_dim; self.max_action = max_action
        self.layer1 = nn.Linear(input_dim, hidden_dim); self.bn1 = nn.BatchNorm1d(hidden_dim)
        self.lstm = nn.LSTM(hidden_dim, rnn_hidden_dim, batch_first=True)
        self.layer2 = nn.Linear(rnn_hidden_dim, hidden_dim); self.bn2 = nn.BatchNorm1d(hidden_dim)
        self.mean_layer = nn.Linear(hidden_dim, output_dim); self.log_std_layer = nn.Linear(hidden_dim, output_dim)
    def forward(self, x, h=None, c=None):
        batch_size, seq_len, _ = x.shape; x = x.view(batch_size * seq_len, -1); h1 = F.relu(self.bn1(self.layer1(x)))
        h1_rnn = h1.view(batch_size, seq_len, -1)
        if h is None or c is None: lstm_out, (h_n, c_n) = self.lstm(h1_rnn)
        else: lstm_out, (h_n, c_n) = self.lstm(h1_rnn, (h, c))
        lstm_out_flat = lstm_out.contiguous().view(batch_size * seq_len, -1); h2 = F.relu(self.bn2(self.layer2(lstm_out_flat)))
        mean = self.mean_layer(h2).view(batch_size, seq_len, -1); log_std = self.log_std_layer(h2).view(batch_size, seq_len, -1)
        log_std = torch.clamp(log_std, min=-20, max=2); std = torch.exp(log_std); return mean, std, (h_n, c_n)
    def sample(self, state_seq, h=None, c=None):
        mean, std, (h_n, c_n) = self.forward(state_seq, h, c); dist = Normal(mean, std); z = dist.rsample()
        action = self.max_action * torch.tanh(z)
        log_prob = dist.log_prob(z) - torch.log(self.max_action * (1 - torch.tanh(z).pow(2)) + 1e-6)
        log_prob = log_prob.sum(axis=-1, keepdim=True); return action, log_prob, (h_n, c_n)
class Critic_RNN(nn.Module):
    def __init__(self, input_dim, action_dim, hidden_dim, rnn_hidden_dim, output_dim=1):
        super(Critic_RNN, self).__init__(); self.rnn_hidden_dim = rnn_hidden_dim
        self.state_layer1 = nn.Linear(input_dim, hidden_dim); self.state_bn1 = nn.BatchNorm1d(hidden_dim)
        self.lstm = nn.LSTM(hidden_dim, rnn_hidden_dim, batch_first=True)
        self.action_layer1 = nn.Linear(action_dim, hidden_dim)
        self.combo_layer1 = nn.Linear(rnn_hidden_dim + hidden_dim, hidden_dim); self.combo_bn1 = nn.BatchNorm1d(hidden_dim)
        self.combo_layer2 = nn.Linear(hidden_dim, output_dim)
    def forward(self, state_seq, action_seq, h=None, c=None):
        batch_size, seq_len, _ = state_seq.shape; state_flat = state_seq.view(batch_size * seq_len, -1)
        s_h1 = F.relu(self.state_bn1(self.state_layer1(state_flat))); s_h1_rnn = s_h1.view(batch_size, seq_len, -1)
        if h is None or c is None: lstm_out, _ = self.lstm(s_h1_rnn)
        else: lstm_out, _ = self.lstm(s_h1_rnn, (h, c))
        action_flat = action_seq.view(batch_size * seq_len, -1); a_h1 = F.relu(self.action_layer1(action_flat))
        lstm_out_flat = lstm_out.contiguous().view(batch_size * seq_len, -1); combined = torch.cat([lstm_out_flat, a_h1], dim=1)
        c_h1 = F.relu(self.combo_bn1(self.combo_layer1(combined))); q_value = self.combo_layer2(c_h1)
...
class SAC_Agent:
    def __init__(self, config):
        self.config = config; self.model_type = config['MODEL_TYPE']; self.gamma = config['GAMMA']
        self.tau = config['TAU']; self.max_action = config['MAX_ACTION']
        ...
        self.target_critic_1.load_state_dict(self.critic_1.state_dict()); self.target_critic_2.load_state_dict(self.critic_2.state_dict())
        self.actor_optimizer = optim.SGD(self.actor.parameters(), lr=config['ACTOR_LR'], momentum=0.9, weight_decay=config['L2_LAMBDA'])
        self.critic_optimizer = optim.SGD(list(self.critic_1.parameters()) + list(self.critic_2.parameters()), lr=config['CRITIC_LR'], momentum=0.9, weight_decay=config['L2_LAMBDA'])
        self.target_entropy = -torch.prod(torch.Tensor(ACTION_DIM).to(device)).item()
        self.log_alpha = torch.zeros(1, requires_grad=True, device=device)
        self.alpha_optimizer = optim.SGD([self.log_alpha], lr=config['ALPHA_LR'], momentum=0.9)
        self.alpha = self.log_alpha.exp().item(); self.actor_h = None; self.actor_c = None
    def select_action(self, state, deterministic=False):
        if self.model_type == 'RNN':
            if state.ndim == 2: state_tensor = torch.tensor(state, dtype=torch.float32).to(device).unsqueeze(0)
            elif state.ndim == 1: state_tensor = torch.tensor(state, dtype=torch.float32).to(device).unsqueeze(0).unsqueeze(0)
            else: state_tensor = state
        else: state_tensor = torch.tensor(state, dtype=torch.float32).to(device).unsqueeze(0)
        self.actor.eval()
        with torch.no_grad():
            if self.model_type == 'RNN':
                mean, std, (h_n, c_n) = self.actor(state_tensor, self.actor_h, self.actor_c)
                self.actor_h, self.actor_c = h_n, c_n; mean, std = mean[:, -1, :], std[:, -1, :]
            else: mean, std = self.actor(state_tensor)
            if deterministic: action = self.max_action * torch.tanh(mean)
            else: dist = Normal(mean, std); z = dist.sample(); action = self.max_action * torch.tanh(z)
        self.actor.train(); return action.detach().cpu().numpy()[0]
    def reset_rnn_hidden_state(self): self.actor_h, self.actor_c = None, None
    def update(self, batch):
        if self.model_type == 'RNN': self.update_rnn(batch)
        else: self.update_mlp(batch)
    def update_mlp(self, batch):
        states, actions, rewards, next_states, dones = batch
        self.critic_1.train(); self.critic_2.train(); self.target_critic_1.eval(); self.target_critic_2.eval(); self.actor.eval()
        with torch.no_grad():
            next_actions, next_log_probs = self.actor.sample(next_states)
            target_q1 = self.target_critic_1(next_states, next_actions); target_q2 = self.target_critic_2(next_states, next_actions)
            target_q = torch.min(target_q1, target_q2); y = rewards + self.gamma * (1.0 - dones) * (target_q - self.alpha * next_log_probs)
        current_q1 = self.critic_1(states, actions); current_q2 = self.critic_2(states, actions)
        critic_loss = F.mse_loss(current_q1, y) + F.mse_loss(current_q2, y)
        self.critic_optimizer.zero_grad(); critic_loss.backward(); self.critic_optimizer.step()
        self.actor.train(); self.critic_1.eval(); self.critic_2.eval()
        pi_actions, log_probs = self.actor.sample(states); q1_pi = self.critic_1(states, pi_actions); q2_pi = self.critic_2(states, pi_actions)
        min_q_pi = torch.min(q1_pi, q2_pi); actor_loss = (self.alpha * log_probs - min_q_pi).mean()
        self.actor_optimizer.zero_grad(); actor_loss.backward(); self.actor_optimizer.step()
        alpha_loss = -(self.log_alpha * (log_probs.detach() + self.target_entropy)).mean()
        self.alpha_optimizer.zero_grad(); alpha_loss.backward(); self.alpha_optimizer.step(); self.alpha = self.log_alpha.exp().item()
        polyak_update(self.target_critic_1, self.critic_1, self.tau); polyak_update(self.target_critic_2, self.critic_2, self.tau)
    def update_rnn(self, batch):
        ...
        self.critic_optimizer.zero_grad(); critic_loss.backward(); self.critic_optimizer.step()
        self.actor.train(); self.critic_1.eval(); self.critic_2.eval()
        pi_actions, log_probs, _ = self.actor.sample(states); q1_pi = self.critic_1(states, pi_actions); q2_pi = self.critic_2(states, pi_actions)
        min_q_pi = torch.min(q1_pi, q2_pi); actor_loss = (self.alpha * log_probs - min_q_pi).mean()
        self.actor_optimizer.zero_grad(); actor_loss.backward(); self.actor_optimizer.step()
        alpha_loss = -(self.log_alpha * (log_probs.detach() + self.target_entropy)).mean()
        self.alpha_optimizer.zero_grad(); alpha_loss.backward(); self.alpha_optimizer.step(); self.alpha = self.log_alpha.exp().item()
...
def run_training_rl(config, train_data, data_info):
    ...
    if config['MODEL_TYPE'] == 'RNN':
        ...
        get_state_fn = get_state_RNN; state_start_idx = config['RNN_SEQUENCE_LENGTH'] - 1
    else:
        replay_buffer = ReplayBuffer(config['BUFFER_SIZE'], config['BATCH_SIZE'])
        get_state_fn = get_state_MLP; state_start_idx = 3
    if config['PREFILL_BUFFER']: replay_buffer.prefill(train_data)
    max_episodes = config['MAX_EPISODES']; data_length = len(train_data)
    max_steps = min(300, data_length - (state_start_idx + 2))
    episode_rewards = []; episode_mse = []
    for episode in range(max_episodes):
        max_start_idx = data_length - max_steps - (state_start_idx + 2)
        if max_start_idx <= state_start_idx: print("Dataset too short for training."); break
        start_idx = np.random.randint(state_start_idx, max_start_idx)
        current_data = train_data.iloc[start_idx : start_idx + max_steps + 5].reset_index(drop=True)
        agent.reset_rnn_hidden_state()
        episode_reward = 0; episode_total_mse = 0; step_count = 0; prev_predicted = None
        for step in range(max_steps):
            step_count += 1; t = state_start_idx + step
            if config['MODEL_TYPE'] == 'RNN': action_state = get_state_RNN(current_data, t)
            else: action_state = get_state_MLP(current_data, t)
            action = agent.select_action(action_state)
            actual_row = current_data.iloc[t + 1]; actual_action = get_action_from_row(actual_row)
            pos, vel, acc = get_current_features(get_state_RNN(current_data, t))
            pred_vel = (action - pos) / 1.0; pred_acc = (pred_vel - vel) / 1.0; pred_jerk = (pred_acc - acc) / 1.0
            actual_jerk = get_jerk_RNN(current_data, t + 1)
            reward = calculate_reward(action, actual_action, pred_jerk, actual_jerk,
                                      prev_predicted, get_action_from_row(current_data.iloc[t]),
                                      is_terminal=False, use_jerk=config['USE_JERK_PENALTY'], jerk_weight=config['JERK_WEIGHT'])
            done = False
            if config['MODEL_TYPE'] == 'MLP':
                state = get_state_MLP(current_data, t); next_state = get_state_MLP(current_data, t + 1)
                replay_buffer.add(state, action, reward, next_state, done)
            else:
                state_9d = get_state_RNN(current_data, t); next_state_9d = get_state_RNN(current_data, t + 1)
                replay_buffer.add(state_9d, action, reward, next_state_9d, done)
            episode_reward += reward; episode_total_mse += np.mean((action - actual_action)**2); prev_predicted = action
        episode_rewards.append(episode_reward); episode_mse.append(episode_total_mse / step_count)
        if len(replay_buffer) >= config['BATCH_SIZE'] * (config['RNN_SEQUENCE_LENGTH'] if config['MODEL_TYPE'] == 'RNN' else 1):
            for _ in range(5):
                batch = replay_buffer.sample()
                if batch is not None: agent.update(batch)
        if (episode + 1) % 50 == 0:
...
def evaluate_rl_agent(agent, config, test_data, data_info):
    print("Evaluating RL Agent..."); test_size = len(test_data)
    if config['MODEL_TYPE'] == 'RNN':
        state_idx = config['RNN_SEQUENCE_LENGTH']; state = [get_state_RNN(test_data, i) for i in range(state_idx)]; agent.reset_rnn_hidden_state()
    else: state_idx = 3; state = get_state_MLP(test_data, state_idx)
    predicted_trajectory = []; actual_trajectory = []
    for i in tqdm(range(state_idx, test_size)):
        noise_vec = np.random.randn(*state.shape) * config['ROBUSTNESS_NOISE_LEVEL'] if config['ROBUSTNESS_NOISE_LEVEL'] > 0 else 0.0
        ...
        else: action = agent.select_action(action_state, deterministic=True)
        actual_row = test_data.iloc[i]; actual_action = get_action_from_row(actual_row)
        predicted_trajectory.append(action); actual_trajectory.append(actual_action)
        if config['MODEL_TYPE'] == 'RNN':
            pos, vel, acc = get_current_features(get_state_RNN(test_data, i-1))
            new_vel = (action - pos) / 1.0; new_acc = (new_vel - vel) / 1.0
            next_state_9d = np.concatenate([action, new_vel, new_acc]); state.append(next_state_9d); state.pop(0)
        else:
            pos, vel, acc = get_current_features(state[-9:])
            new_vel = (action - pos) / 1.0; new_acc = (new_vel - vel) / 1.0
            new_state_9d = np.concatenate([action, new_vel, new_acc]); state = np.concatenate([state[9:], new_state_9d])
    return calculate_metrics(predicted_trajectory, actual_trajectory, data_info)
def calculate_metrics(predicted, actual, data_info):
    predicted_traj = np.array(predicted); actual_traj = np.array(actual)
    predicted_actual = np.zeros_like(predicted_traj); actual_actual = np.zeros_like(actual_traj)
    for i, joint_name in enumerate(['joint1', 'joint2', 'joint3']):
        sigma = data_info[joint_name]['sigma']; mu = data_info[joint_name]['mu']
        predicted_actual[:, i] = predicted_traj[:, i] * sigma + mu
        actual_actual[:, i] = actual_traj[:, i] * sigma + mu
    mse_actual = np.mean((predicted_actual - actual_actual)**2); rmse_actual = np.sqrt(mse_actual)
...
def main():
    ...
    if 'BC' in CONFIG['EXPERIMENT_NAME']: # 修改: 捕获 '2_BC_MLP'
